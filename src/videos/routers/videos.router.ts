...
import { videoCreateInputDtoValidation, videoUpdateInputDtoValidation } from '../validation/videoInputDtoValidation';
...
videosRouter
  ...
  .get('', (req: Request, res: Response<Video[]>) => res.status(HttpStatus.Ok).send(db.videos))

  /*GET-запрос для поиска видео по ID при помощи URI-параметров.*/
  .get('/:id', (req: Request<{ id: string }, Video, {}, {}>, res: Response<Video | null | unknown>) => {
    /*Ищем видео в БД.*/
    ...
    if (!video) {
      res.status(HttpStatus.NotFound).send(createErrorMessages([{ field: 'id', message: 'Video was not found' }]));
      return;
    }
  ...
  .put('/:id', (req: Request<{ id: string }, {}, VideoUpdateInputDto, {}>, res: Response) => {
    ...
    const errors = videoUpdateInputDtoValidation(req.body, Number(req.params.id));
